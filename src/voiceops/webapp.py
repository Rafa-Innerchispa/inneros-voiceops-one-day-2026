...
class VoiceOpsHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if self.path == "/api/boson/token":
            ...
            if boson_configured():
                minted = mint_client_secret()
                if minted.get("ok"):
                    self._send_json(minted)
                else:
                    self._send_json(
                        {
                            "ok": False,
                            "error": minted.get("error", "client_secrets failed"),
                            "ws_url": "/ws/higgs",
                            "connection_mode": "relay_fallback",
                ...
                self._send_json(
                    {
                        "ok": False,
                        "error": "BOSON NOT CONNECTED - API KEY REQUIRED",
                        "ws_url": "/ws/higgs",
                        "connection_mode": "browser_fallback",
                        "model": "higgs-realtime",
                        "sample_rate": 24000,
                        "ready": False,
    ...
    def _handle_ws_higgs(self) -> None:
        """RFC6455 endpoint: relay to Boson upstream when configured, else text-only browser TTS fallback."""
        sec_key = self.headers.get("Sec-WebSocket-Key", "")
        if not sec_key:
            self.send_error(HTTPStatus.BAD_REQUEST, "Missing Sec-WebSocket-Key")
            return

        from .websocket_server import compute_accept_key, read_ws_frame, encode_ws_frame
        from .adapters.boson_client import boson_configured, relay_boson_websocket
        from .adapters.higgs_realtime import HiggsRealtimeSession
        ...
        def browser_send_text(text: str) -> None:
            with write_lock:
        ...
        if boson_configured():
            ...
            inbound: queue.Queue[bytes | None] = queue.Queue()

            def browser_send_raw(payload: bytes) -> None:
                with write_lock:
                    self.wfile.write(encode_ws_frame(1, payload))
                    self.wfile.flush()

            def browser_recv() -> bytes | None:
                try:
        ...
        # Boson unavailable: text-only fallback — browser TTS, never fake Higgs PCM audio.
        session = HiggsRealtimeSession()
        browser_send_text(
            json.dumps(
                {
        ...
        try:
            while True:
                ...
                if msg_type in ("input_audio_buffer.speech_started", "response.cancel"):
                    browser_send_text(
                        json.dumps(
                            {
                ...
                if msg_type in ("conversation.item.create", "response.create", "user_utterance"):
                    text = (
